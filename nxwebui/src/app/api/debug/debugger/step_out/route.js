...
// POST /api/debug/debugger/step_out
export async function POST(request) {
  try {
    ...
    // Call PySpider API to step out
    try {
      const response = await axios.post(`${process.env.PYSPIDER_API_URL}/debug/debugger/step_out`, {
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'Failed to execute step out',
    ...
    console.error('Error executing step out:', error);
    return NextResponse.json(
      { error: 'Failed to execute step out' },
      { status: 500 }
    );
