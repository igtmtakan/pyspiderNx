...
document.addEventListener('DOMContentLoaded', function() {
    ...
    methods: {
      ...
      updateCounter() {
        const updateCounters = () => {
          ...
          // Update avg time separately
          // Call updateAvgTime() separately to avoid race conditions
          setTimeout(() => this.updateAvgTime(), 500);
      ...
      updateAvgTime() {
        ...
        $.get("/api/avg_time", (rawData) => {
          console.log('Avg time raw data received:', rawData);
          ...
          // Flag to track if any project data was updated
          let dataUpdated = false;

          // Loop through all projects in the avg time data
          for (let projectName in data) {
            ...
            // Update the project data in this.projects
            if (this.projects[projectName]) {
              ...
              dataUpdated = true;
              console.log('Updated Vue data model for', projectName);
            }
          }

          // If we're using DataTables, redraw the table to reflect the updated data
          if (dataUpdated && window.projectsDataTable) {
            console.log('Redrawing DataTables to reflect updated data');
            try {
              window.projectsDataTable.draw(false); // false = don't reset pagination
            } catch (e) {
              console.error('Error redrawing DataTables:', e);
            }
          }
        });
      }
  ...
  // Make mountedApp available globally for DataTables drawCallback
  window.mountedApp = mountedApp;

  // Set up periodic avg time updates (every 5 seconds)
  setInterval(() => {
    mountedApp.updateAvgTime();
  }, 5000);
});
